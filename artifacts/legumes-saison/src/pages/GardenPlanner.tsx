...
import { AlertCircle, Download, RotateCcw, Info } from 'lucide-react';
...
export default function GardenPlanner() {
  ...
  const handleDrop = (r: number, c: number) => {
    if (draggingId) {
      const newGrid = [...grid];
      newGrid[r] = [...newGrid[r]];
      newGrid[r][c] = { plantId: draggingId };
      setGrid(newGrid);
      setDraggingId(null);
    }
  };
  ...
  const getEmptyCellSuggestions = (r: number, c: number) => {
    if (grid[r][c].plantId) return null;
    const neighborIds = [
      grid[r-1]?.[c]?.plantId,
      grid[r+1]?.[c]?.plantId,
      grid[r]?.[c-1]?.plantId,
    ...
    neighborIds.forEach(nid => {
      ...
      if (p && p.associations.benefiques.length > 0) {
        p.associations.benefiques.forEach(bid => {
  ...
  return (
    <div className="min-h-[100dvh] flex flex-col bg-background pb-24">
      ...
      <main className="max-w-7xl mx-auto w-full px-4 flex flex-col md:flex-row gap-8">
        {/* Left Palette */}
        <div className="w-full md:w-72 space-y-4 shrink-0 flex flex-col h-[calc(100vh-200px)]">
          <div className="bg-card p-4 rounded-xl border border-border shadow-sm flex flex-col flex-1 min-h-0">
            ...
            <div className="overflow-y-auto flex-1 pr-2 space-y-2">
              <div className="flex flex-wrap gap-2">
                {filteredPalette.map(p => (
                  ...
                  >
                    <div className={`w-2.5 h-2.5 rounded-full shrink-0 ${p.categorie === 'Herbes aromatiques' ? 'bg-green-400' : p.categorie.includes('Fruits') ? 'bg-red-400' : 'bg-amber-400'}`} />
                  ...
                  return (
                    <div
                      ...
                      className={`
                        ...
                        ${cell.plantId ? 'bg-primary/5 border-2 border-primary/20' : 'border-2 border-dashed border-border hover:border-primary/50 hover:bg-muted/50'}
                    ...
                    >
                      {plant ? (
                        <>
                          <div className={`w-3 h-3 rounded-full absolute top-2 left-2 ${plant.categorie === 'Herbes aromatiques' ? 'bg-green-400' : plant.categorie.includes('Fruits') ? 'bg-red-400' : 'bg-amber-400'}`} />
                          <span className="font-serif font-medium text-sm md:text-base text-center px-2 leading-tight">{plant.nom}</span>
                          <button 
                            onClick={(e) => {
                              e.stopPropagation();
                              const newGrid = [...grid];
                              newGrid[r] = [...newGrid[r]];
                              newGrid[r][c] = { plantId: null };
                              setGrid(newGrid);
                            }}
